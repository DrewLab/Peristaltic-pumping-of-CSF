...
public class PVS_3D_matched_p {
  ...
  public static Model run(String arg) {
	...
	model.label("PVS_3D_matched"+ arg +".mph");
	
	//create parameters
	String paramsFile = modelPath + "/Peristalsis_3D_matched_params.txt";
	model = setparams(model, paramsFile); // set parameters from the paramFile
	model.param().set("p1", "0.0" + arg+ "[Torr]"); //set the pressure difference
    ...
    model.component("comp1").physics("vfeq").feature("weak1").selection().set(3, 7, 15, 19);
    ...
    model.component("comp1").physics("vfeq").feature("weak1").set("weakExpression", "step1(t/teq)*p1/po*TBCf/Leq^2");
    ...
    model.result().export("data1").set("filename", modelPath + "/pvsMatchedResultsP" +arg +".txt");
  ...
  public static void main(String[] args) {
		...
		System.out.println("No arguments given: running with a pressue difference of 0.02 [Torr]");
		Model model = run("2");
	}
	else{
