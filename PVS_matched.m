...
% Generates Fig 5e,5f
% To be used for the output of PVS_3D_matched.class
%
...
%% read the data
FileName = 'pvsMatchedResults.txt';
...
h = text(mean([t(1) t(end)]), mean([part_pos_um(1) part_pos_um(end)])- 0.025*diff(ax.YLim), ['Mean speed ' num2str(round(mean_vel,3)) '\mum/s'],...
...
set(gca, 'FontSize',14)
xlabel('time(s)')
saveas(fig, 'PVS_5mm_trajectory.png')
saveas(fig, 'PVS_5mm_trajectory.pdf')
...
set(gca,'YColor','b')
set(gca, 'FontSize',24)
saveas(fig, 'FigS2c.png')
saveas(fig, 'FigS2c.pdf')